...
class ProviderCrawl():
    def __init__(self, seed, escore):
        self.seed = seed
        self.escore = escore

    def __aws_fetch(self):
        amazon_seed = self.seed['aws']
        AWS_URL = "{}/offers/v1.0/aws/index.json".format(amazon_seed)
        ...
        for offers in data['offers']:
            ...
            try:
                nURL = "{}{}".format(amazon_seed, data['offers'][offers]['currentVersionUrl'])
                print(nURL)
        ...
        with open('../../data/crawled_aws.json', 'w') as fp:
            json.dump(dump, fp)

    def __google_fetch(self):
        # GOO_URL = "https://cloudpricingcalculator.appspot.com/static/data/pricelist.json"
        goo_seed = self.seed['goo']
        try:
            r = requests.get(goo_seed, timeout=0.5)
        except Exception as inst:
            print("Failed to fetch data for AWS with %s and %s" % (inst.args, type(inst)))
            sys.exit(1)
        ...
        with open('../../data/crawled_goo.json', 'w') as fp:
            json.dump(data, fp)
        return data

    def __aws_parse(self):
        if not os.path.isfile("../../data/crawled_aws.json"):
            return 0
    ...
    def crawl(self):
        if 'aws' in self.seed:
            try:
                ...
                print("Exception while fetching aws provider data {} with {}".format(type(inst), inst.args))
                return 1
        if 'goo' in self.seed:
            try:
                self.__google_fetch()
            except Exception as inst:
                print("Exception while fetching google provider data {} with {}".format(type(inst), inst.args))
                return 1
        return 0

    def index(self, indexable, index):
        index_connection = ESCore(self.esendpoint)
        for d in indexable:
            index_connection.pushData(index, doc_type='d', body=d)
        return 0